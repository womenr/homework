...
public class AgentService {
	...
	public void sortToFiles() {
		...
		List<AgentBasicInfo> agentBasicInfoList = agentBasicInfoMapper.orderByAsc("shop_number");
		//用作存储单个店番募集人的数据（原本打算逐行写入文件，考虑到效率，将方法改成list）
		...
		for (int i = 0; i < agentBasicInfoList.size() -1; i++) { //少遍历一次，因为用上了i+1.
			...
			if (number == agentBasicInfoList.get(i+1).getShopNumber()) {
				...
				MyFileHandler.writeCsvByList(directory, "agent_"+ number, dataList);
				dataList.clear();
			}
		}
		...
		MyFileHandler.writeCsvByList(directory, "agent_"+ lastOne.getShopNumber(), dataList);
	}
